...
export class Coachs implements OnInit {
  ...
  async handleDelete(id: string) {
    this.coachService.canBeDeleted(id).subscribe({
      next: async (res: any) => {
        if (res) {
          let result = await Swal.fire({
            title: 'Voulez vous supprimer cet entraineur',
            icon: 'question',
          ...
          Swal.fire('Info','Vous ne pouvez pas supprimer cette enraineur car il n\'a pas terminer ses séances programmer','info');
        }
      },
    });
  }
